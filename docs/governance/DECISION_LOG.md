...
| 2026-07-11 | Assign `ops.floridarampandliftops.com` to this repo; preserve existing Render-backed hosts; use Clerk email/password for Phase B and defer Google OAuth. | Active | Removes sibling-host collision risk and avoids unnecessary production Google credentials. Clerk development and production now align on email code plus required password with Google disabled; deploy status is complete. | G2 is complete. G3 may link the Vercel project, but `ops` DNS and every deploy remain separately gated. |
