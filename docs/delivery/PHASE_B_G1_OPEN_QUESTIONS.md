...
| Q5 | Who is on the internal reviewer list allowed to receive the pilot URL under the tightly-controlled-circulation rule? | **Preview circulation after operator-only G5 verification** | Operator | Named reviewer list (roles suffice; no personal contact details in repo) | Unanswered |
| Q6 | Who reviews and approves ops-domain Terms/Privacy copy (counsel and/or operator), and when? | **Preview circulation and G6** (legal publication gate) | Operator / counsel | Reviewer identity + target date + approved disposition satisfying the [`../legal/LEGAL_PRIVACY_DOCTRINE.md`](../legal/LEGAL_PRIVACY_DOCTRINE.md) publication gate | Unanswered |
...
- G5 circulation posture — 2026-07-11 — Operator: allow an operator-only,
  non-circulated Preview deployment and verification before Q5/Q6 are
  complete; keep Q5 reviewer authorization and Q6 legal approval as hard
  blockers before sharing the Preview URL with anyone else. This records the
  circulation boundary only and is not a G5 deploy approval.
...
- Q5 and Q6 gate circulation, not deployment mechanics: a Preview may be
  deployed and verified by the operator alone before the reviewer list or
  legal copy exist, but its URL must not be shared with any reviewer or third
  party until both questions are answered and their evidence is recorded.
