...
- Completed Phase B gate G2 (2026-07-11): operator assigned `ops.floridarampandliftops.com` to this repo while preserving the Render-backed apex, `www`, `admin`, `client`, `contractor`, and `platform` hosts; approved Clerk email/password and deferred Google OAuth; Google was disabled in development to match production; Clerk deployment status now reports complete with DNS/SSL/mail/OAuth green and no pending records/providers. No `ops` DNS, Vercel config, env write, or deploy occurred.
- Added the proposed Phase 4 Resend transactional email plan: reserves `mail.floridarampandliftops.com` (singular `lift`) for application sending; separates Clerk identity email from Resend transactional email; defines sender identities, proposed env names, DNS/secret/runtime gates, rollback, and acceptance criteria; and defers the canonical application origin until sibling-host ownership is resolved. Documentation only; no provider, DNS, secret, dependency, runtime, webhook, or email-send action authorized.
...
- Resolved Phase B Q3 credential provenance (2026-07-10): a names-only audit
  ...
  local file was deleted, and Git history remained empty. At that time G2 still
  awaited its own `proceed`; instance deletion/replacement remains separately gated.
