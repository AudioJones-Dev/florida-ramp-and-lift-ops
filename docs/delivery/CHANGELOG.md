...
- G2 partially executed under the operator's CLI-first `proceed` (2026-07-10/11), recorded in `docs/delivery/G2_EXECUTION_RECORD.md`: Clerk CLI 2.1.0; production instance confirmed (was on a Clerk placeholder domain — never configured); domain set to `floridarampandliftops.com` via `change_domain`; five required CNAMEs identified (additive; no overlap with the live contractor-portal records); DNS application **blocked** on stale Cloudflare tokens in Doppler (operator to provide a new scoped token or add records manually); new production Secret Key rotated and escrowed directly to Doppler (`florida-ramp-and-lift/prd` → `CLERK_SECRET_KEY`, value never printed or persisted). No Vercel writes, no deploy, no instance deletion.
