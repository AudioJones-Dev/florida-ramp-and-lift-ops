...
- G2 partially executed under the operator's CLI-first `proceed` (2026-07-10/11), recorded in `docs/delivery/G2_EXECUTION_RECORD.md`: Clerk CLI 2.1.0; production instance confirmed (was on a Clerk placeholder domain — never configured); domain set to `floridarampandliftops.com` via `change_domain`; five required CNAMEs identified (additive; values read live from Clerk status, not committed); DNS application **blocked** on stale stored Cloudflare tokens (operator to provide a new scoped token or add records manually); new production Secret Key rotated and the current post-domain-change Publishable Key captured, both escrowed directly to the approved production secret-manager configuration (`CLERK_SECRET_KEY`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`; values never printed or persisted). No Vercel writes, no deploy, no instance deletion.
